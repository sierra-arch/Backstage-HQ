...
3. **Onboarding Form Engine (public intake wizard) + Client Journey Engine (`clients.stage` state machine)** — replaces manual client creation, wires the automated stage transitions. The state machine here is what the Home Dashboard will eventually read for its "is my business okay" signal, so get the transition/event model right now even though the dashboard itself is last.
